'''
History Encoder V5 = V3(contrastive student-teacher)에서 teacher 를 context 조건부
VAE(CVAE)로 교체.

  1. Teacher 입력 = student 와 동일한 window 의 history_t + o_{t+1}
  2. context c = MLP-mixer(history_t)        (teacher 소유 mixer 로 history 압축)
  3. encoder(c, o_{t+1}) -> (t_mu, t_logvar)
  4. decoder(c, t_mu)    -> o_{t+1} recon    (t_mu 직접 decode — 스펙)
  5. student 경로 / contrastive / vel loss / optimizer 순서는 v3 와 동일
  6. v2/v3 는 무수정 유지 — 이 클래스는 PPOHistV3 상속 (additive)

데이터 흐름: teacher history = student 가 그 스텝에 본 encoder_obs 그대로(rollout 에
자동 저장), o_{t+1} = v2 부터 캡처하던 next_obs_target. 따라서 rollout/storage/obs
config 는 v2/v3 와 완전히 동일 — 오버라이드 불필요.

의미: decoder 가 context 를 받으므로 z_t 는 "history 로 예측 불가능한 o_{t+1} 의
잔차 정보"만 담게 되고, student z_s 는 그 잔차 표현에 정렬된다.

주의: v3 ckpt 는 teacher 구조가 달라 strict load 불가 (actor/critic 은 호환).

Launch: +exp=locomotion_hist_v5 +obs=loco/leggedloco_obs_history_encoder (v2/v3 와 동일 obs)
'''
import torch
...
from humanoidverse.agents.modules.ppo_hist_v3_modules import (
    PPOActorWithStudentEncoderContrastive,
)
from humanoidverse.agents.modules.ppo_hist_v5_modules import TeacherContextCVAEContrastive
from humanoidverse.agents.ppo_hist_v3.ppo_hist_v3 import PPOHistV3


class PPOHistV5(PPOHistV3):
    """PPOHistV3 + teacher 를 (context=mixer(history), o_{t+1}) CVAE 로 교체."""

    # ---- models: actor/critic 은 v3 와 동일, teacher 만 context-CVAE ----
    def _setup_models_and_optimizer(self):
        ...
        tc = self.config.teacher_config
        self.teacher = TeacherContextCVAEContrastive(
            history_dim=self.algo_obs_dim_dict[self.encoder_obs_key],
            obs_dim=self.algo_obs_dim_dict[self.recon_target_key],
            latent_dim=self.latent_dim,
            encoder_struct=self.encoder_struct,
            context_dim=int(tc.context_dim),
            mixer_hidden_dims=tuple(tc.mixer_hidden_dims),
            mixer_channel_hidden_dims=tc.get('mixer_channel_hidden_dims', None),
            enc_hidden_dims=tuple(tc.enc_hidden_dims),
            dec_hidden_dims=tuple(tc.dec_hidden_dims),
        ...
        self.teacher_optimizer = optim.Adam(
            self.teacher.parameters(), lr=float(tc.learning_rate))

    # rollout/storage 는 v2 그대로 상속:
    #   encoder_obs(=teacher history 입력)는 obs 그룹이라 자동 저장,
    #   next_obs_target(o_{t+1})/base_vel_target 은 v2 가 캡처.

    # ---- update: v3 의 _update_ppo 복제, teacher 호출만 (history, o_{t+1}) 로 교체 ----
    def _update_ppo(self, policy_state_dict, loss_dict):
        ...
        # ---- CHANGED(v5): teacher = CVAE(context=mixer(history_t), o_{t+1}) ----
        target_next = policy_state_dict['next_obs_target']
        valid_mask = (~policy_state_dict['dones'].bool()).float()
        recon, t_mu, t_logvar = self.teacher(
            policy_state_dict[self.encoder_obs_key], target_next)
        ...
        # ---- student supervised + contrastive (v3 과 동일) ----
        ...
        # 두 backward 를 모두 끝낸 뒤 teacher step (v3 과 동일한 순서)
